...
import {SectionList, Text, StyleSheet, ImageSourcePropType, SectionListRenderItem, SectionListData} from "react-native";
...
interface Props {
    ...
    itemHeight: number;
    itemsPerRow: number;
    renderSectionHeader?: (info: {section: SectionListData<any>}) => JSX.Element;
...
const InteractiveSectionList: React.FC<Props> = ({
    ...
    const getItemLayout = sectionListGetItemLayout({
        ...
        getSeparatorHeight: () => /*StyleSheet.hairlineWidth*/ 0,
    ...
    const handleRenderSectionHeader: any = (info: {section: SectionListData<any>}) => {
        ...
        return (
            <Text
                ...
                onLayout={({nativeEvent}) => setSectionHeaderHeight(nativeEvent.layout.height)}
            >
                {info.section.title}
            </Text>
        );
            ...
            onScroll={({nativeEvent}) => {
                ...
                const itemIndex = Math.floor(
                    nativeEvent.contentOffset.y /
                        (itemHeight +
                            ((sectionHeaderHeight + StyleSheet.hairlineWidth) * sectionsFeed.length) /
                                (flattenedList.length * itemsPerRow)),
                );
